...
function commitMount(domElement, type, newProps, internalInstanceHandle) {
  console.log('commitMount', domElement, type);
...
function commitUpdate(domElement, updatePayload, type, oldProps, newProps, internalInstanceHandle) {
  console.log('commitUpdate', domElement, updatePayload)
...
function insertBefore(parentInstance, child, beforeChild) {
  console.log('insertBefore', child, beforeChild);
...
var classComponentUpdater = {
  ...
  enqueueSetState: function (inst, payload, callback) {
    ...
    var groupName = 'enqueueSetState - ' + expirationTime;
    console.group(groupName);
  ...
  enqueueReplaceState: function (inst, payload, callback) {
    ...
    var groupName = 'enqueueReplaceState - ' + expirationTime;
    console.group(groupName);
  ...
  enqueueForceUpdate: function (inst, callback) {
    ...
    var groupName = 'enqueueForceUpdate - ' + expirationTime;
    console.group(groupName);
...
function beginWork(current$$1, workInProgress, renderExpirationTime) {
  var groupName = 'beginWork - ' + getComponentName(workInProgress);
...
function commitAllHostEffects() {
  console.group('commitAllHostEffects')
  while (nextEffect !== null) {
    ...
    switch (primaryEffectTag) {
      case Placement:
        {
          console.group('commitPlacement');
          commitPlacement(nextEffect);
          console.groupEnd('commitPlacement');
      ...
      case PlacementAndUpdate:
        {
          ...
          console.group('commitWork - ' + getComponentName(nextEffect));
          commitWork(_current, nextEffect);
          console.groupEnd('commitWork - ' + getComponentName(nextEffect));
          break;
        }
      case Update:
        {
      ...
      case Deletion:
        {
          console.group('commitDeletion');
          commitDeletion(nextEffect);
          console.groupEnd('commitDeletion');
          break;
        }
    }
  ...
  console.groupEnd('commitAllHostEffects');
}
...
function commitRoot(root, finishedWork) {
  ...
  var groupName = 'commitRoot - ' + committedExpirationTime;
  console.group(groupName);
...
function completeUnitOfWork(workInProgress) {
  var groupName = 'completeUnitOfWork - ' + getComponentName(workInProgress)
  workTrackingLogs && console.group(groupName);

  // Attempt to complete the current unit of work, then move to the
  // next sibling. If there are no more siblings, return to the
  // parent fiber.
  while (true) {
    workTrackingLogs && console.log('Completing ' + getComponentName(workInProgress) + '...');
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      if (next !== null) {
        ...
        // back here again.
        workTrackingLogs && console.groupEnd(groupName);
        return next;
      }
...
function performUnitOfWork(workInProgress) {
  var groupName = 'performUnitOfWork - ' + getComponentName(workInProgress);
  workTrackingLogs && console.group(groupName);
...
function renderRoot(root, isYieldy, isExpired) {
  ...
  var groupName = 'renderRoot - ' + expirationTime;
...
function retrySuspendedRoot(root, fiber, suspendedTime) {
  if (enableSuspense) {
    var groupName = 'retrysuspendedRoot - ' + suspendedTime;
    console.group(groupName);
...
function deferredUpdates(fn) {
  ...
  var groupName = 'deferredUpdates - ' + expirationContext;
  console.group(groupName);
...
function scheduleCallbackWithExpirationTime(expirationTime) {
  ...
  // timeout soon enough
  // console.log('scheduleCallbackWithExpirationTime - ', expirationTime);
...
function onSuspend(root, finishedWork, suspendedExpirationTime, rootExpirationTime, msUntilTimeout) {
  var groupName = 'onSuspend - ' + suspendedExpirationTime;
  console.group(groupName);
  ...
  if (enableSuspense && msUntilTimeout === 0 && !shouldYield()) {
    ...
    console.log('schedulingTimeout of', suspendedExpirationTime, 'for', msUntilTimeout, '(id: ' + root.timeoutHandle + ')');
  }
  console.groupEnd(groupName);
}
...
function onTimeout(root, finishedWork, suspendedExpirationTime) {
  if (enableSuspense) {
    var groupName = 'onTimeout - ' + suspendedExpirationTime;
    console.group(groupName);
...
function requestWork(root, expirationTime) {
  var groupName = 'requestWork - ' + expirationTime;
  console.group(groupName);
  // console.log('root:', root.containerInfo); // Useful if there are multiple roots
...
function performWorkOnRoot(root, expirationTime, isExpired) {
  var groupName = 'performWorkOnRoot - ' + expirationTime;
  console.group(groupName);
  // console.log('root:', root.containerInfo); // Useful if there are multiple roots
  ...
  if (deadline === null || isExpired) {
    ...
    if (finishedWork !== null) {
      ...
      if (enableSuspense && timeoutHandle !== noTimeout) {
        ...
        console.log('Canceling timeout', root.timeoutHandle, '...');
...
function completeRoot(root, finishedWork, expirationTime) {
  // var groupName = 'completeRoot - ' + expirationTime;
  // console.group(groupName);
...
ReactRoot.prototype.render = function (children, callback) {
  console.group('ReactRoot.render');
  ...
  console.groupEnd('ReactRoot.render');
  return work;
};
