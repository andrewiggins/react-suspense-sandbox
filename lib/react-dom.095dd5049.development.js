...
// ==== BEGIN CUSTOM METHOD TRACER IMPLEMENTATION ==== //
/**
 * @type {Record<string, Record<string, boolean>>}
 */
var MethodTracerConfig = {
  renderRoot: {
    collapsed: true
  }
};
 ...
 */
MethodTracer.prototype.enter = function enter(name, idParam) {
  var methodConfig = MethodTracerConfig[name];

  var consoleArgs = [name];
  ...
  if (methodConfig && methodConfig.collapsed) {
    ...
    console.group.apply(console, consoleArgs);
  }

  return this.stack.push(this.getId(name, idParam));
};

MethodTracer.prototype.exit = function exit() {
  console.groupEnd();
  return this.stack.pop();
};

MethodTracer.prototype.log = function log() {
 ...
 */
MethodTracer.prototype.unwindOnError = function unwindOnError(error, name, idParam) {
  var id = this.getId(name, idParam);
  console.log('Error caught in "' + this.lastId() + '". Rewinding to "' + id + '"');
  console.log('Error:', error);

  while (this.lastId() !== id) {
    this.exit();
  }
};
 ...
 * @param {any} [idParam] An optional parameter identifying this method call
 */
MethodTracer.prototype.getId = function (name, idParam) {
 ...
 * @private
 */
MethodTracer.prototype.lastId = function lastId() {
  return this.stack[this.stack.length - 1];
};

var ReactTracer = new MethodTracer();
// ==== END CUSTOM METHOD TRACER IMPLEMENTATION ==== //
...

{
  ...
  if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
    ...
    // Temporarily disable the dev complexity in invokeGuardedCallbackDev
...
function getCurrentFiberStackInDevOrNull() {
  {
    if (current === null) {
      return null;
    }
...
function initWrapperState(element, props) {
  {
    ReactControlledValuePropTypes.checkPropTypes('input', props, getCurrentFiberStackInDevOrNull);
...
function checkSelectPropTypes(props) {
  ReactControlledValuePropTypes.checkPropTypes('select', props, getCurrentFiberStackInDevOrNull);
...
function initWrapperState$2(element, props) {
  var node = element;
  {
    ReactControlledValuePropTypes.checkPropTypes('textarea', props, getCurrentFiberStackInDevOrNull);
...
function validateProperty(tagName, name) {
  ...
  if (rARIACamel.test(name)) {
    ...
    if (correctName == null) {
      warning$1(false, 'Invalid ARIA attribute `%s`. ARIA attributes follow the pattern aria-* and must be lowercase.%s', name, getStackAddendum());
      warnedProperties[name] = true;
      return true;
    }
    // aria-* attributes should be lowercase; suggest the lowercase version.
    if (name !== correctName) {
      warning$1(false, 'Invalid ARIA attribute `%s`. Did you mean `%s`?%s', name, correctName, getStackAddendum());
      warnedProperties[name] = true;
      return true;
    }
  }
  ...
  if (rARIA.test(name)) {
    ...
    if (name !== standardName) {
      warning$1(false, 'Unknown ARIA attribute `%s`. Did you mean `%s`?%s', name, standardName, getStackAddendum());
      warnedProperties[name] = true;
      return true;
    }
  }
...
function warnInvalidARIAProps(type, props) {
  ...
  if (invalidProps.length === 1) {
    warning$1(false, 'Invalid aria prop %s on <%s> tag. ' + 'For details, see https://fb.me/invalid-aria-prop%s', unknownPropString, type, getStackAddendum());
  } else if (invalidProps.length > 1) {
    warning$1(false, 'Invalid aria props %s on <%s> tag. ' + 'For details, see https://fb.me/invalid-aria-prop%s', unknownPropString, type, getStackAddendum());
  }
}
...
function validateProperties$1(type, props) {
  ...
  if (props != null && props.value === null && !didWarnValueNull) {
    ...
    if (type === 'select' && props.multiple) {
      warning$1(false, '`value` prop on `%s` should not be null. ' + 'Consider using an empty array when `multiple` is set to `true` ' + 'to clear the component or `undefined` for uncontrolled components.%s', type, getStackAddendum$1());
    } else {
      warning$1(false, '`value` prop on `%s` should not be null. ' + 'Consider using an empty string to clear the component or `undefined` ' + 'for uncontrolled components.%s', type, getStackAddendum$1());
    }
  }
}
  ...
  validateProperty$1 = function (tagName, name, value, canUseEventSystem) {
    ...
    if (canUseEventSystem) {
      ...
      if (registrationName != null) {
        warning$1(false, 'Invalid event handler property `%s`. Did you mean `%s`?%s', name, registrationName, getStackAddendum$2());
        warnedProperties$1[name] = true;
        return true;
      }
      if (EVENT_NAME_REGEX.test(name)) {
        warning$1(false, 'Unknown event handler property `%s`. It will be ignored.%s', name, getStackAddendum$2());
        warnedProperties$1[name] = true;
        return true;
      }
      ...
      if (INVALID_EVENT_NAME_REGEX.test(name)) {
        warning$1(false, 'Invalid event handler property `%s`. ' + 'React events use the camelCase naming convention, for example `onClick`.%s', name, getStackAddendum$2());
      }
      warnedProperties$1[name] = true;
      return true;
    }
    ...
    if (lowerCasedName === 'is' && value !== null && value !== undefined && typeof value !== 'string') {
      warning$1(false, 'Received a `%s` for a string attribute `is`. If this is expected, cast ' + 'the value to a string.%s', typeof value, getStackAddendum$2());
      warnedProperties$1[name] = true;
      return true;
    }

    if (typeof value === 'number' && isNaN(value)) {
      warning$1(false, 'Received NaN for the `%s` attribute. If this is expected, cast ' + 'the value to a string.%s', name, getStackAddendum$2());
      warnedProperties$1[name] = true;
      return true;
    }
    ...
    if (possibleStandardNames.hasOwnProperty(lowerCasedName)) {
      ...
      if (standardName !== name) {
        warning$1(false, 'Invalid DOM property `%s`. Did you mean `%s`?%s', name, standardName, getStackAddendum$2());
        warnedProperties$1[name] = true;
        return true;
      }
      ...
      warning$1(false, 'React does not recognize the `%s` prop on a DOM element. If you ' + 'intentionally want it to appear in the DOM as a custom ' + 'attribute, spell it as lowercase `%s` instead. ' + 'If you accidentally passed it from a parent component, remove ' + 'it from the DOM element.%s', name, lowerCasedName, getStackAddendum$2());
      warnedProperties$1[name] = true;
      return true;
    }
...
var warnUnknownProperties = function (type, props, canUseEventSystem) {
  ...
  if (unknownProps.length === 1) {
    warning$1(false, 'Invalid value for prop %s on <%s> tag. Either remove it from the element, ' + 'or pass a string or number value to keep it in the DOM. ' + 'For details, see https://fb.me/react-attribute-behavior%s', unknownPropString, type, getStackAddendum$2());
  } else if (unknownProps.length > 1) {
    warning$1(false, 'Invalid values for props %s on <%s> tag. Either remove them from the element, ' + 'or pass a string or number value to keep them in the DOM. ' + 'For details, see https://fb.me/react-attribute-behavior%s', unknownPropString, type, getStackAddendum$2());
  }
};
...
var getStackInDevOrNull = function () {
  return '';
};
...

{
  ...
  warnForInvalidEventListener = function (registrationName, listener) {
    if (listener === false) {
      warning$1(false, 'Expected `%s` listener to be a function, instead got `false`.\n\n' + 'If you used to conditionally omit it with %s={condition && value}, ' + 'pass %s={condition ? value : undefined} instead.%s', registrationName, registrationName, registrationName, getCurrentFiberStackInDevOrNull());
    } else {
      warning$1(false, 'Expected `%s` listener to be a function, instead got a value of `%s` type.%s', registrationName, typeof listener, getCurrentFiberStackInDevOrNull());
    }
  };
...
function setInitialDOMProperties(tag, domElement, rootContainerElement, nextProps, isCustomComponentTag) {
  for (var propKey in nextProps) {
    ...
    if (propKey === STYLE) {
      ...
      setValueForStyles(domElement, nextProp, getStackInDevOrNull);
...
function updateDOMProperties(domElement, updatePayload, wasCustomComponentTag, isCustomComponentTag) {
  ...
  for (var i = 0; i < updatePayload.length; i += 2) {
    ...
    if (propKey === STYLE) {
      setValueForStyles(domElement, propValue, getStackInDevOrNull);
    } else if (propKey === DANGEROUSLY_SET_INNER_HTML) {
...

{
  ...
  validateDOMNesting = function (childTag, childText, ancestorInfo) {
    ...
    var addendum = getCurrentFiberStackInDevOrNull();
...
function prepareUpdate(domElement, type, oldProps, newProps, rootContainerInstance, hostContext) {
  ...
  var updatePayload = diffProperties(domElement, type, oldProps, newProps, rootContainerInstance);
...
function getMaskedContext(workInProgress, unmaskedContext) {
...

  {
    ...
    checkPropTypes_1(contextTypes, context, 'context', name, getCurrentFiberStackInDevOrNull);
  }
...
function recordElapsedActualRenderTime(fiber) {
  ...
  {
    !(fiber === fiberStack$1.pop()) ? warning$1(false, 'Unexpected Fiber (%s) popped.', getComponentName(fiber)) : void 0;
  }
...
function checkShouldComponentUpdate(workInProgress, oldProps, newProps, oldState, newState, newContext) {
  ...
  if (typeof instance.shouldComponentUpdate === 'function') {
    ...
    {
      !(shouldUpdate !== undefined) ? warning$1(false, '%s.shouldComponentUpdate(): Returned undefined instead of a ' + 'boolean value. Make sure to return true or false.', getComponentName(workInProgress) || 'Component') : void 0;
    }
...
function checkClassInstance(workInProgress) {
  ...
  var type = workInProgress.type;
  {
    ...
    if (type.prototype && type.prototype.isPureReactComponent && typeof instance.shouldComponentUpdate !== 'undefined') {
      warning$1(false, '%s has a method called shouldComponentUpdate(). ' + 'shouldComponentUpdate should not be used when extending React.PureComponent. ' + 'Please extend React.Component if shouldComponentUpdate is used.', getComponentName(workInProgress) || 'A pure component');
    }
    ...
    if (typeof instance.getSnapshotBeforeUpdate === 'function' && typeof instance.componentDidUpdate !== 'function' && !didWarnAboutGetSnapshotBeforeUpdateWithoutDidUpdate.has(type)) {
      ...
      warning$1(false, '%s: getSnapshotBeforeUpdate() should be used with componentDidUpdate(). ' + 'This component defines getSnapshotBeforeUpdate() only.', getComponentName(workInProgress));
    }
...
function callComponentWillMount(workInProgress, instance) {
  ...
  if (oldState !== instance.state) {
    {
      warning$1(false, '%s.componentWillMount(): Assigning directly to this.state is ' + "deprecated (except inside a component's " + 'constructor). Use setState instead.', getComponentName(workInProgress) || 'Component');
    }
...

{
  ...
  warnForMissingKey = function (child) {
    ...
    var currentComponentErrorInfo = 'Each child in an array or iterator should have a unique ' + '"key" prop. See https://fb.me/react-warning-keys for ' + 'more information.' + (getCurrentFiberStackInDevOrNull() || '');
    if (ownerHasKeyUseWarning[currentComponentErrorInfo]) {
      return;
    }
    ownerHasKeyUseWarning[currentComponentErrorInfo] = true;

    warning$1(false, 'Each child in an array or iterator should have a unique ' + '"key" prop. See https://fb.me/react-warning-keys for ' + 'more information.%s', getCurrentFiberStackInDevOrNull());
  };
}
...
function throwOnInvalidObjectType(returnFiber, newChild) {
  if (returnFiber.type !== 'textarea') {
    var addendum = '';
    {
      addendum = ' If you meant to render a collection of children, use an array ' + 'instead.' + (getCurrentFiberStackInDevOrNull() || '');
    }
...
function warnOnFunctionType() {
  var currentComponentErrorInfo = 'Functions are not valid as a React child. This may happen if ' + 'you return a Component instead of <Component /> from render. ' + 'Or maybe you meant to call this function rather than return it.' + (getCurrentFiberStackInDevOrNull() || '');
  ...
  warning$1(false, 'Functions are not valid as a React child. This may happen if ' + 'you return a Component instead of <Component /> from render. ' + 'Or maybe you meant to call this function rather than return it.%s', getCurrentFiberStackInDevOrNull() || '');
}
  ...
  function reconcileChildrenIterator(returnFiber, currentFirstChild, newChildrenIterable, expirationTime) {
...

    {
      ...
      if (newChildrenIterable.entries === iteratorFn) {
        !didWarnAboutMaps ? warning$1(false, 'Using Maps as children is unsupported and will likely yield ' + 'unexpected results. Convert it to a sequence/iterable of keyed ' + 'ReactElements instead.%s', getCurrentFiberStackInDevOrNull()) : void 0;
    ...
    workInProgress.tag = FunctionalComponent;
    {
      ...
      if (workInProgress.ref !== null) {
        ...
        if (!didWarnAboutStatelessRefs[warningKey]) {
          ...
          warning$1(false, 'Stateless function components cannot be given refs. ' + 'Attempts to access this ref will fail.%s%s', info, getCurrentFiberStackInDevOrNull());
        }
      }
...
function updateContextProvider(current$$1, workInProgress, renderExpirationTime) {
...

  {
    ...
    if (providerPropTypes) {
      checkPropTypes_1(providerPropTypes, newProps, 'prop', 'Context.Provider', getCurrentFiberStackInDevOrNull);
    }
  }
...
function beginWork(current$$1, workInProgress, renderExpirationTime) {
  ReactTracer.log('beginWork', '-', getComponentName(workInProgress));
          ...
          if (current$$1 !== null) {
            ...
            var snapshot = instance.getSnapshotBeforeUpdate(prevProps, prevState);
            {
              ...
              if (snapshot === undefined && !didWarnSet.has(finishedWork.type)) {
                ...
                warning$1(false, '%s.getSnapshotBeforeUpdate(): A snapshot value (or null) ' + 'must be returned. You have returned undefined.', getComponentName(finishedWork));
              }
            }
    ...
    if (typeof ref === 'function') {
    ...
    } else {
      {
        if (!ref.hasOwnProperty('current')) {
          warning$1(false, 'Unexpected ref object provided for %s. ' + 'Use either a ref-setter function or React.createRef().%s', getComponentName(finishedWork), getStackByFiberInDevAndProd(finishedWork));
        }
      }
...
function commitAllHostEffects() {
  ReactTracer.enter('commitAllHostEffects')
      ...
      case PlacementAndUpdate:
        {
          ...
          ReactTracer.enter('commitWork', getComponentName(nextEffect));
...
function commitRoot(root, finishedWork) {
  ...
  ReactTracer.enter('commitRoot', committedExpirationTime);
...
function completeUnitOfWork(workInProgress) {
  ReactTracer.enter('completeUnitOfWork', getComponentName(workInProgress));

  // Attempt to complete the current unit of work, then move to the
  // next sibling. If there are no more siblings, return to the
  // parent fiber.
  while (true) {
    ReactTracer.log('Completing', getComponentName(workInProgress));
...
function performUnitOfWork(workInProgress) {
  ReactTracer.enter('performUnitOfWork', getComponentName(workInProgress));
...
function renderRoot(root, isYieldy, isExpired) {
  ...
  ReactTracer.enter('renderRoot', expirationTime);
  ...
  do {
    try {
      ...
      ReactTracer.unwindOnError(thrownValue, 'renderRoot', expirationTime);
...
function retrySuspendedRoot(root, fiber, suspendedTime) {
  if (enableSuspense) {
    ReactTracer.enter('retrysuspendedRoot', suspendedTime);
...
function deferredUpdates(fn) {
  ...
  ReactTracer.enter('deferredUpdates', expirationContext);
...
function scheduleCallbackWithExpirationTime(expirationTime) {
  // Can be called multiple times but doesn't guarentee a callback is scheduled
  // Whether or not a calback is scheduled depends on if the existing one will
  // timeout soon enough
  // ReactTracer.log('scheduleCallbackWithExpirationTime', '-', expirationTime);
...
function onSuspend(root, finishedWork, suspendedExpirationTime, rootExpirationTime, msUntilTimeout) {
  ReactTracer.enter('onSuspend', suspendedExpirationTime);
  ...
  if (enableSuspense && msUntilTimeout === 0 && !shouldYield()) {
    ...
    ReactTracer.log('schedulingTimeout of', suspendedExpirationTime, 'for', msUntilTimeout, 'ms (id:', root.timeoutHandle, ')');
  }
  ReactTracer.exit();
}
...
function onTimeout(root, finishedWork, suspendedExpirationTime) {
  if (enableSuspense) {
    ReactTracer.enter('onTimeout', suspendedExpirationTime);
...
function requestWork(root, expirationTime) {
  ReactTracer.enter('requestWork', expirationTime);
  // ReactTracer.log('root:', root.containerInfo); // Useful if there are multiple roots

  addRootToSchedule(root, expirationTime);
  if (isRendering) {
    // Prevent reentrancy. Remaining work will be scheduled at the end of
    // the currently rendering batch.
    ReactTracer.log('already rendering');
    ReactTracer.exit();
    return;
  }
...
function performWorkOnRoot(root, expirationTime, isExpired) {
  ReactTracer.enter('performWorkOnRoot', expirationTime);
  // ReactTracer.log('root:', root.containerInfo); // Useful if there are multiple roots
  ...
  if (deadline === null || isExpired) {
    ...
    if (finishedWork !== null) {
      ...
      if (enableSuspense && timeoutHandle !== noTimeout) {
        ...
        ReactTracer.log('Canceling timeout', root.timeoutHandle);
...
function completeRoot(root, finishedWork, expirationTime) {
  // ReactTracer.enter('completeRoot', expirationTime);
