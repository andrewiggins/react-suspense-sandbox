...
function postMountWrapper$2(element, props) {
	ReactTracer.log('postMountWrapper', element); // value="" should make a value attribute (#6219)
...
const Hydrating =
	/*                    */
	0b000000000000001000000000000; // You can change the rest (and add more).
...
const Forked =
	/*                       */
	0b000000100000000000000000000; // Static tags describe aspects of a fiber that are not specific to a render,
...
const PassiveStatic =
	/*                */
	0b000100000000000000000000000; // Flag used to identify newly inserted fibers. It isn't reset after commit unlike `Placement`.
...
const scheduleCallback$2 = unstable_scheduleCallback;
...
let attemptContinuousHydration$1;
function setAttemptContinuousHydration(fn) {
	attemptContinuousHydration$1 = fn;
}
let attemptHydrationAtCurrentPriority$1;
...
const ReactCurrentBatchConfig$3 = ReactSharedInternals.ReactCurrentBatchConfig; // TODO: can we stop exporting these?
...
function setInitialProperties(domElement, tag, rawProps) {
	...
	switch (tag) {
		case 'input':
			...
			track(domElement);
			postMountWrapper$3(domElement, rawProps, false);
			break;
...
function diffHydratedProperties(
	...
	switch (tag) {
		case 'input':
			...
			// up necessary since we never stop tracking anymore.
			track(domElement);
			postMountWrapper$3(domElement, rawProps, true);
			break;
...
function commitUpdate(
	...
	ReactTracer.log('commitUpdate', domElement, updatePayload); // Apply the diff to the DOM node.
...
function flushSyncCallbacks() {
	if (!isFlushingSyncQueue && syncQueue !== null) {
		...
		let i = 0;
		const previousUpdatePriority = getCurrentUpdatePriority$1();
...
function updateEffectImpl(fiberFlags, hookFlags, create, deps) {
	const hook = updateWorkInProgressHook();
	const nextDeps = deps === undefined ? null : deps;
	let destroy = undefined; // currentHook is null when rerendering after a render phase state update.
...
function requestUpdateLane(fiber) {
	...
	const updateLane = getCurrentUpdatePriority$1();
	...
	switch (exitStatus) {
		...
		case RootSuspendedWithDelay: {
...

			{
				...
				const timeElapsedMs = now$1() - eventTimeMs;
...
function commitRoot(root, recoverableErrors, transitions) {
	...
	const previousUpdateLanePriority = getCurrentUpdatePriority$1();
	const prevTransition = ReactCurrentBatchConfig.transition;
...
var ReactVersion = '18.3.0-next-43f0eaebe-20230301';
